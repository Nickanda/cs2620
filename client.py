    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            ...
            if version != "0":
                ...
                logged_in_user = words[2]
                new_messages = int(words[3])
                ...
                print(f"Error: {' '.join(words[2:])}")
                messagebox.showerror("Error", f"{' '.join(words[2:])}")
            elif words[1] == "refresh_home":
                # Refresh home screen with updated data
                state_data = int(words[2])
                current_state = "home"
            elif words[1] == "messages":
                # Update messages screen
                if len(words) > 2:
                    state_data = [word.split("_") for word in words[2].split("\0")]
                else:
                    state_data = []
